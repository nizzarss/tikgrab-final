...
async function downloadMedia(format, label) {
    if (!currentVideo) {
        showToast('Data video belum tersedia.', 'error');
        return;
    }
    ...
    if (format === 'mp4-nwm') {
        url = currentVideo.play || '';
        ext = 'mp4';
    } else if (format === 'mp4-hd') {
        url = currentVideo.hdplay || currentVideo.play || '';
        ext = 'mp4';
    } else if (format === 'mp3') {
        url = currentVideo.music || '';
        ext = 'mp3';
    }

    if (!url || !/^https?:\/\//i.test(url)) {
        showToast('Link download tidak tersedia. Coba proses ulang videonya.', 'error');
        return;
    }

    const filename = `TikGrab-${Date.now()}.${ext}`;
    showToast(`Menyiapkan ${label}...`, 'info');

    // First try fetch -> Blob -> local object URL. This can trigger a real
    // browser download when the media server permits CORS.
    try {
        ...
        showToast('Download dimulai. Cek folder Downloads.', 'success');
        return;
    } catch (error) {
        console.warn('Direct blob download tidak tersedia:', error);
    }

    // Cross-origin servers may block fetch/CORS. Try the native download
    // attribute as a second option before falling back to opening the media.
    try {
        const link = document.createElement('a');
        link.href = url;
        link.download = filename;
        ...
        showToast('Mencoba direct download...', 'info');
    } catch (error) {
        console.warn('Native download gagal:', error);
        window.open(url, '_blank', 'noopener,noreferrer');
        showToast('Server tidak mengizinkan direct download dari browser.', 'info');
    }
}
